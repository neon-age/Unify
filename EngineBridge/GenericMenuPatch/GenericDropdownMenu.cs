...
namespace UnityEditor.UIElements.Hijacked
{
    ...
    public class GenericDropdownMenu : IGenericMenu
    {
        ...
        internal ScrollView scrollView => m_ListView.Q<ScrollView>();
        ...
        static readonly ObjectPool<ListView> s_ListPool = new (() =>
        {
            ...
            listView.AddToClassList(containerInnerUssClassName);
        ...
        internal GenericDropdownMenu(bool allowSubmenus)
        {
            ...
            m_MenuContainer.AddToClassList(containerOuterUssClassName);

            m_MenuContainer.Add(m_OuterContainer);

            m_TitleBar = new VisualElement();
            m_TitleBar.AddToClassList(titleAreaUssClassName);
            m_TitleBar.AddToClassList(hiddenClassName);
            ...
            m_Back = new VisualElement();
            m_Back.AddToClassList(backUssClassName);
            m_Back.AddToClassList(hiddenClassName);
            ...
            m_HeaderButtons = new VisualElement();
            m_HeaderButtons.AddToClassList(titleButtonContainerUssClassName);
        ...
        void ReleasePooledItems(MenuItem root)
        {
            ...
            var R_root_element = new R_VisualElement(root.element);

            R_root_element.pseudoStates = 0;
            ...
            R_root_element.m_CallbackRegistry.m_BubbleUpCallbacks.GetCallbackListForWriting().Clear();
            R_root_element.m_CallbackRegistry.m_TrickleDownCallbacks.GetCallbackListForWriting().Clear();
        ...
        internal void Hide(bool giveFocusBack = false, bool hideParent = true)
        {
            ...
            if (m_TargetElement != null)
            {
                ...
                var R_m_TargetElement = new R_VisualElement(m_TargetElement);
                R_m_TargetElement.pseudoStates ^= PseudoStates.Active;
        ...
        void OnPointerDown(PointerDownEvent evt)
        {
            ...
            UpdateSelection(new R_EventBase(evt).elementTarget);
            ...
            {
                new R_IPanel(m_MenuContainer.panel).PreventCompatibilityMouseEvents(evt.pointerId);
            }
        ...
        {
            var isPlayer = menuContainer.panel?.contextType == ContextType.Player;
        ...
        void ChangeSelectedIndex(int newIndex)
        {
            ...
            {
                var R_element = new R_VisualElement(m_Current.children[m_PreviousIndex].element);
                R_element.pseudoStates &= ~PseudoStates.Hover;
            }
            ...
            {
                var R_element = new R_VisualElement(m_Current.children[newIndex].element);
                R_element.pseudoStates |= PseudoStates.Hover;
        ...
        int GetSelectedIndex()
        {
            ...
            {
                var R_element = new R_VisualElement(item.element);
                if ((R_element.pseudoStates & PseudoStates.Hover) == PseudoStates.Hover)
                {
                    return index;
                }
        ...
        internal static VisualElement CreateSeparator()
        { 
            var element = new VisualElement()
            {
                pickingMode = PickingMode.Ignore,
            };
            element.AddToClassList(separatorUssClassName);
            return element;
        }
        ...
        VisualElement BuildItem(string name, bool isChecked, bool isEnabled, bool isSubmenu, object data, Texture2D icon, string tooltip)
        {
            ...
            if (isChecked)
            {
                var R_item = new R_VisualElement(item);
                R_item.pseudoStates |= PseudoStates.Checked;
            }
        ...
        internal void UpdateItem(string itemName, bool isChecked)
        {
            ...
            if (isChecked)
                R_element.pseudoStates |= PseudoStates.Checked;
            else
                R_element.pseudoStates &= ~PseudoStates.Checked;
        }
        ...
        public void DropDown(Rect position, VisualElement targetElement = null, bool anchored = false)
        {
            ...
            m_PanelRootVisualContainer = new R_VisualElement(m_TargetElement).GetRootVisualContainer();
            ...
            var R_targetElement_computedStyle = new R_ComputedStyle(m_TargetElement);
            m_MenuContainer.style.fontSize = R_targetElement_computedStyle.fontSize;
            m_MenuContainer.style.unityFont = R_targetElement_computedStyle.unityFont;
            m_MenuContainer.style.unityFontDefinition = R_targetElement_computedStyle.unityFontDefinition;
            ...
            if (targetElement != null)
            {
                var R_targetElement = new R_VisualElement(targetElement);
                R_targetElement.pseudoStates |= PseudoStates.Active;
            }
        }
